module Main exposing (Flags, Model, Msg(..), Order, curtain, curtainTexture, green, init, main, orders, subscriptions, update, view)

import Browser
import CardRepresentation exposing (CardsMsg, cardName, toPath)
import Cards exposing (Card(..), Face(..), Suit(..))
import Dealer exposing (DrawState(..), dealNextCard, iterateDrawState)
import Debug exposing (log)
import Deck exposing (ShuffledDeck, fullDeck, getCards, map, randomDeck, take)
import Element exposing (..)
import Element.Background
import Element.Border
import Html exposing (Html)
import MagicTrick exposing (Game, ProperSizedDeck, SlicedDeck(..), createProperSizedDeck, handOut, unwrapSlicedDeck)
import Maybe
import Random
import Task
import Time
...
init _ =
    let
        aGame =
            Result.map handOut (createProperSizedDeck [])
    in
    ...
    case msg of
        ShuffleDeck newDeck ->
            let
                ...
                dummy =
                    Deck.map (log "Drawn " << cardName) drawnCards
                ...
                theGame =
                    Result.map handOut properSizedDeck
            in
            ( { model
                | game = theGame
                , visualizedGame = theGame
              }
            , Cmd.none
            )
        ...
        Tick newTime ->
            let
                ...
                currentGame =
                    .visualizedGame model

                nextDrawState =
                    iterateDrawState model.drawState

                newVisualizedGame =
                    dealNextCard currentGame currentDrawState
            in
            ( { model | drawState = nextDrawState, timeDelta = timeDelta, visualizedGame = newVisualizedGame }, Cmd.none )
...
view model =
    let
        renderCard size card =
            image [ alignBottom, width (fill |> maximum size), centerY ] { src = toPath card, description = cardName card }

        cardSize =
            200

        magician =
            image [ alignBottom, width (fill |> maximum 500) ] { src = "Background.png", description = "The Magician" }

        beBlue =
            Element.Background.color <| Element.rgb 0 0 1.0
        ...
        takeCardByDrawState drawState =
            ...
            case game of
                Result.Ok validGame ->
                    validGame |> f |> unwrapSlicedDeck |> List.head |> Maybe.withDefault Back
